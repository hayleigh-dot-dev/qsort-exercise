...
withClass class button =
  { button
  | attrs = A.class class :: button.attrs
  }
...
toHtml button =
  let
    attrs =
      [ Maybe.map (Ui.Colour.toString >> (++) "bg-" >> A.class) button.colour
      , Maybe.map (Ui.Colour.darken >> Ui.Colour.toString >> (++) "hover:bg-" >> A.class) button.colour
